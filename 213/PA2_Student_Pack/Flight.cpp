...
bool Flight::addTicket(const Ticket &ticket) {
   Ticket *ptr; 
  int count_eco=0,count_bus=0;
    for (;ptr!=NULL;++ptr){

        if(ptr->getTicketType()==economy)
            count_eco++;
        else if(ptr->getTicketType()==business)
            count_bus++;    
    }

    if(ticket.getTicketType()==economy)
      {  if(count_eco<getEconomyCapacity())
            return true;
      }
    else if(ticket.getTicketType()==economy)
      {  if(count_bus<getBusinessCapacity())
            return true;
      }
    return false;                  
    /* TODO */
