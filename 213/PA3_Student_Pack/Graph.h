...
private:
bool vertex_exist(const std::string& VertexName){
    for (int i=0; i< vertexList.size();i++){
        if(vertexList.at(i).name==VertexName)
            return true;
    }
    return false;
};
bool vertex_exist(const std::string& VertexName)const{
    for (int i=0; i< vertexList.size();i++){
        if(vertexList.at(i).name==VertexName)
            return true;
    }
...
bool edge_exist(const int vertex1, int vertex2 ){

    for (int i=0; i< edgeList.size();i++){
        if(edgeList.at(i).vertexId0==vertex1 && edgeList.at(i).vertexId1==vertex2)
            return true;
        else if(edgeList.at(i).vertexId1==vertex1 && edgeList.at(i).vertexId0==vertex2) 
            return true;   
    }
...
int vertex_index(const std::string& Vertex){

    for(int i=0; i< vertexList.size(); i++){
        if(vertexList.at(i).name==Vertex)
            return i;
    }
}

int vertex_index_new(const std::string& Vertex)const{
...
bool in_the_vector(const std::vector<int> vector,int index){
...
int find_max_edge_weight(std::vector<int> &new_list){
    int weight=0;
    ...
    for(int i=0; i< new_list.size()-1;i++ ){
        for (int j=0;j<edgeList.size();j++){
            ...
            if(pointer->vertexId0==new_list.at(i)&& pointer->vertexId1==new_list.at(i+1)||
               pointer->vertexId1==new_list.at(i)&& pointer->vertexId0==new_list.at(i+1)){
                   
                   if(weight<pointer->weight){
...
void remove_edge(std::vector<int> &new_list,int weight){
    bool v1_exists,v2_exists;

    for(int i=0;i<edgeList.size();i++)
    {
        GraphEdge* pointer=&edgeList.at(i);
        if(pointer->weight==weight)
        {
            v1_exists=in_the_vector(new_list,pointer->vertexId0);
            v2_exists=in_the_vector(new_list,pointer->vertexId1);
            
            if(v1_exists==true && v2_exists==true)
            {   
                pointer->masked=true;
            }
            else
            { 
            v1_exists=in_the_vector(new_list,pointer->vertexId1);
            v2_exists=in_the_vector(new_list,pointer->vertexId0);
